...
function normalizeHexColor(value) {
  ...
  if (!/^#[0-9a-fA-F]{6}$/.test(text)) return '#1f2937'
...
function normalizeShortcutsLenient(rawList, issues, profileId) {
  ...
  for (const raw of rawList) {
    ...
    result[key] = {
      ...
      actionType: raw.actionType === 'url' ? 'url' : raw.actionType === 'app' ? 'app' : 'command',
...
function validateShortcutArrayStrict(rawList, profileId, errors) {
  ...
  rawList.forEach((item, index) => {
    ...
    if (item.actionType !== 'command' && item.actionType !== 'url' && item.actionType !== 'app') {
      errors.push(`profiles.${profileId}[${index}].actionType doit être "command", "url" ou "app".`)
    }
